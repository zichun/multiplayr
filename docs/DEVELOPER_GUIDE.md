...
*   **Inspect cards & icons instead of guessing**: Icons (0–100 viewBox geometry) and card text (em-based) can't be trusted from source. Before and after editing any `*Assets.ts` icon map or `CardDefinition`, run the headless inspector — `npm run inspect -- icon <module> <id>` for an ASCII preview and `npm run inspect -- card <module> <export>` for a legibility/structure lint. It's far cheaper than spinning up webpack + a browser. See [DESIGN_GUIDE.md §11](file:///c:/repos/multiplayr/docs/DESIGN_GUIDE.md).
